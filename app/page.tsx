...
export default function DownloadPage() {
  ...
  useEffect(() => {
    ...
    if (userAgent.includes("arm64") || userAgent.includes("aarch64")) {
      ...
      // Note: x86_64 detection was removed earlier
      setDeviceType("arm64-v8a") 
  ...
  const handleDownload = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    link.download = `waledpro-${deviceType || 'arm64-v8a'}.apk`; // Default filename if deviceType is null initially
              ...
              {detectionFailed ? (
                <div className="w-full mb-4">
                  ...
                  <div className="relative w-full">
                    ...
                    >
                      <span>
                        {deviceType ? `${deviceType} (${deviceDescriptions[deviceType as keyof typeof deviceDescriptions]})` : 'Select Version'}
                      </span>
                ...
                <p className="text-sm text-gray-400 mb-4">
                  Recommended version: <span className="text-white font-medium">{deviceType || 'Detecting...'}</span>
                </p>
              )}

              <a href={downloadLink} onClick={handleDownload} aria-disabled={!deviceType} className={!deviceType ? 'pointer-events-none opacity-50' : ''}>
                <Button size="lg" className="w-full bg-red-600 hover:bg-red-700 text-white py-6 px-8 text-lg" disabled={!deviceType}>
